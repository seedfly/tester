...
public class PublisherXunlei implements Publisher {
    ...
    public void publish(String dateString) {
        ...
        map.put("wysiwyg", "1");
        map.put("subject", dateString + "迅雷会员账号分享");
        map.put("message",
                dateString + " 迅雷会员账号分享\n[hide]，账号："
                        + RandomString.getMeaningString() + "， 密码："
        ...
        try {
            System.out
                    .println(HttpClientUtil
                            .getPostContent(
                                    "http://vipfxbbs.com/forum.php?mod=post&action=newthread&fid=38&extra=&topicsubmit=yes",
                                    map, COOKIE_STRING));
        } catch (IOException e) {
