...
public class Publisheryouku implements Publisher {
    ...
    public void publish(String dateString) {
        ...
        map.put("wysiwyg", "1");
        map.put("subject", dateString + "优酷vip会员账号分享");
        map.put("message",
                dateString + " 优酷vip会员账号分享\n[hide]，账号："
        ...
        try {
            System.out
                    .println(HttpClientUtil
                            .getPostContent(
                                    "http://vipfxbbs.com/forum.php?mod=post&action=newthread&fid=2&extra=&topicsubmit=yes",
                                    map, COOKIE_STRING));
        } catch (IOException e) {
