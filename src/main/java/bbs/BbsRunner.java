...
public class BbsRunner {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 100; i++) {
			...
			map.put("wysiwyg", "1");
			map.put("subject", dateTools.getSimpleDataString() + "百度度盘会员账号分享");
			map.put("message", dateTools.getSimpleDataString() + " 百度度盘会员账号分享\n[hide] 请使用QQ第三方登陆，账号："
					+ RandomString.getRandomDits(9) + "， 密码：" + RandomString.getRandomString(10) + " [/hide]");
			...
			try {
				System.out.println(HttpClientUtil.getPostContent(
						"http://vipfxbbs.com/forum.php?mod=post&action=newthread&fid=41&extra=&topicsubmit=yes", map,
						COOKIE_STRING));
			} catch (IOException e) {
